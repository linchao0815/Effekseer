	...
	for (int32_t frameIndex = 0; frameIndex < frameCount; frameIndex++)
	{
		...
		for (int32_t faceIndex = 0; faceIndex < faceCount; faceIndex++)
		{
			...
			for (uint32_t i = 0; i < genCount; i++)
			{
				...
				point.Color = PackColor(RandomTriangle<GpuParticles::float4>(random, v0.VColor.ToFloat4(), v1.VColor.ToFloat4(), v2.VColor.ToFloat4()));
			}
			pointIndex += genCount;
		}
	}
...
void GpuParticles::ComputeFrame()
{
	...
	for (auto emitterID : m_newEmitterIDs)
	{
		...
		command.UniformBufferPtrs[0] = m_ubufParamSets[emitter.GetParamID()];
		command.UniformBufferPtrs[1] = m_ubufEmitters[emitterID];
	...
	for (EmitterID emitterID = 0; emitterID < (EmitterID)m_emitters.size(); emitterID++)
	{
		...
		if (emitter.IsAlive())
		{
			...
			if (emitter.IsEmitting())
			{
				...
				{
					emitter.NextEmitCount = (uint32_t)(round(paramSet.EmitPerFrame * emitter.DeltaTime));
...
void GpuParticles::RenderFrame()
{
	...
	for (EmitterID emitterID = 0; emitterID < (EmitterID)m_emitters.size(); emitterID++)
	{
		...
		if (emitter.IsAlive())
		{
			...
			drawParams.VertexUniformBufferPtrs[1] = drawParams.PixelUniformBufferPtrs[1] = m_ubufParamSets[emitter.GetParamID()];
			...
			drawParams.SetTexture(2, colorTex, toWrapType(paramSet.ColorTexWrap), toSamplingType(paramSet.ColorTexFilter));

			Effekseer::Backend::TextureRef normalTex = m_dummyNormalTexture;
			if (auto tex = paramRes.Effect->GetNormalImage(paramSet.NormalTexIndex))
			{
				normalTex = tex->GetBackend();
			}
			drawParams.SetTexture(3, normalTex, toWrapType(paramSet.NormalTexWrap), toSamplingType(paramSet.NormalTexFilter));
